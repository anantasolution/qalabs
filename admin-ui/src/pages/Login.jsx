  ...
  const handleSubmit = async (e) => {
      ...
      if(validateData()){
       ...
       try{
         ...
         const response = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/auth/loginAdmin`,formData,{withCredentials:true})
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-6">
          ...
          <div>
            ...
            <div className='relative flex flex-col'>
              {
                <span onClick={()=>setShowPassword((prev)=>!prev)} className='absolute cursor-pointer top-4 text-gray-600 right-2'>{showPassword?<Eye className='w-5 h-5'></Eye>:<EyeClosed className='w-5 h-5'></EyeClosed>}</span>
              }
              <input
