...
const AddFeedback = ({ placeholder }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFeedback({ name: "", designation: "", reviewMessage: "", profilePicture: null, imagePreview: "" });
      setContent("");
    } catch (error) {
      ...
      <div className="h-[88.5%] overflow-y-auto">
        <div className="w-full p-6 flex justify-center items-center">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full">
            <h1 className="text-2xl font-bold mb-8 text-center">Create New Feedback</h1>
            <form onSubmit={handleSubmit} className="space-y-9">
              {/* Image Upload Section */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Profile Picture</label>
                <div
                  className={`border-2 border-dashed rounded-lg p-4 text-center ${feedback.imagePreview ? "border-green-500" : "border-gray-300"}`}
                  ...
                  {feedback.imagePreview ? (
                    <div className="relative flex justify-center">
                      <div className="w-64 h-64 overflow-hidden rounded-full relative">
                        <img src={feedback.imagePreview} alt="Preview" className="object-cover w-full h-full" />
                        <button
                          type="button"
                          className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full hover:bg-red-600 transition-colors z-10"
                          onClick={() => setFeedback((prev) => ({ ...prev, profilePicture: null, imagePreview: "" }))}
                        >
              ...
              <div>
                <label htmlFor="reviewMessage" className="block text-sm font-medium text-gray-700 mb-2">Review Message</label>
                <JoditEditor
                  id="reviewMessage"
                  ref={editor}
                  value={content}
              ...
              <div className="w-full flex justify-end items-center">
                <button
                  ...
                  className={`px-4 py-2 text-white bg-blue-500 rounded-lg ${isSubmitting ? "opacity-50 cursor-not-allowed" : "hover:bg-blue-600"}`}
                >
